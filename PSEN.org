...
* Introduction
A liquid withdrawal valve on the NLTF (PAB) LN_2 tank has recently developed a leak and needs to be replaced. This engineering note covers modification of the {{{SYS_NAME}}} required to install the replacement valve.
...
#+begin_src python
  if VJ:
    ...
    if CATEGORY == 'Normal':
        ...
        if low_stress:
            print('The piping system qualifies for low stress piping in accordance with FESHM 5031.1.')
